...
Demo::Demo()
	: m_shadow_map_dsv(NULL),
	  ...
	  m_clipmap_texture_opacity_r32_uav(NULL),
	  m_clipmap_texture_opacity_r32_srv(NULL),
	  m_clipmap_texture_opacity_r16_uav(NULL),
	  m_clipmap_texture_opacity_r16_srv(NULL),
	  m_clipmap_texture_illumination_r32_uav(NULL),
	  m_clipmap_texture_illumination_r32_srv(NULL),
	  m_clipmap_texture_illumination_g32_uav(NULL),
	  m_clipmap_texture_illumination_g32_srv(NULL),
	  m_clipmap_texture_illumination_b32_uav(NULL),
	  m_clipmap_texture_illumination_b32_srv(NULL),
	  m_clipmap_texture_illumination_r16g16b16_uav(NULL),
	  m_clipmap_texture_illumination_r16g16b16_srv(NULL),
...
void Demo::Init(ID3D11Device *device, ID3D11DeviceContext *device_context, IDXGISwapChain *swap_chain)
{
...

	{
		...
		D3D11_TEXTURE3D_DESC texture3d_desc;
		texture3d_desc.Width = clipmap_opacity_texture_extent.x;
		texture3d_desc.Height = clipmap_opacity_texture_extent.y;
		texture3d_desc.Depth = clipmap_opacity_texture_extent.z;
		texture3d_desc.MipLevels = 1U;
		texture3d_desc.Format = DXGI_FORMAT_R32_TYPELESS;
		texture3d_desc.Usage = D3D11_USAGE_DEFAULT;
		texture3d_desc.BindFlags = D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_UNORDERED_ACCESS;
		...
		ID3D11Texture3D *clipmap_texture_opacity = NULL;

		HRESULT res_device_create_texture = device->CreateTexture3D(&texture3d_desc, NULL, &clipmap_texture_opacity);
		...
		unordered_access_view_desc.Texture3D.MipSlice = 0U;
		unordered_access_view_desc.Texture3D.FirstWSlice = 0U;
		unordered_access_view_desc.Texture3D.WSize = clipmap_opacity_texture_extent.z;

		assert(NULL == this->m_clipmap_texture_opacity_r32_uav);

		HRESULT res_device_create_unordered_access_view = device->CreateUnorderedAccessView(clipmap_texture_opacity, &unordered_access_view_desc, &this->m_clipmap_texture_opacity_r32_uav);
		assert(SUCCEEDED(res_device_create_unordered_access_view));
		...
		assert(NULL == this->m_clipmap_texture_opacity_r32_srv);

		HRESULT res_device_create_shader_resource_view = device->CreateShaderResourceView(clipmap_texture_opacity, &shader_resource_view_desc, &this->m_clipmap_texture_opacity_r32_srv);
		assert(SUCCEEDED(res_device_create_shader_resource_view));

		clipmap_texture_opacity->Release();
	}

	{
		...
		texture3d_desc.Depth = clipmap_opacity_texture_extent.z;
		texture3d_desc.MipLevels = 1U;
		texture3d_desc.Format = DXGI_FORMAT_R16_FLOAT;
		texture3d_desc.Usage = D3D11_USAGE_DEFAULT;
		texture3d_desc.BindFlags = D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_UNORDERED_ACCESS;
		texture3d_desc.CPUAccessFlags = 0U;
		texture3d_desc.MiscFlags = 0U;
		...
		D3D11_UNORDERED_ACCESS_VIEW_DESC unordered_access_view_desc;
		unordered_access_view_desc.Format = DXGI_FORMAT_R16_FLOAT;
		unordered_access_view_desc.ViewDimension = D3D11_UAV_DIMENSION_TEXTURE3D;
		unordered_access_view_desc.Texture3D.MipSlice = 0U;
		unordered_access_view_desc.Texture3D.FirstWSlice = 0U;
		unordered_access_view_desc.Texture3D.WSize = clipmap_opacity_texture_extent.z;

		assert(NULL == this->m_clipmap_texture_opacity_r16_uav);

		HRESULT res_device_create_unordered_access_view = device->CreateUnorderedAccessView(clipmap_texture_opacity, &unordered_access_view_desc, &this->m_clipmap_texture_opacity_r16_uav);
		assert(SUCCEEDED(res_device_create_unordered_access_view));

		D3D11_SHADER_RESOURCE_VIEW_DESC shader_resource_view_desc;
		shader_resource_view_desc.Format = DXGI_FORMAT_R16_FLOAT;
		shader_resource_view_desc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURE3D;
		shader_resource_view_desc.Texture3D.MostDetailedMip = 0U;
		shader_resource_view_desc.Texture3D.MipLevels = 1U;

		assert(NULL == this->m_clipmap_texture_opacity_r16_srv);

		HRESULT res_device_create_shader_resource_view = device->CreateShaderResourceView(clipmap_texture_opacity, &shader_resource_view_desc, &this->m_clipmap_texture_opacity_r16_srv);
		assert(SUCCEEDED(res_device_create_shader_resource_view));
	...
	{
		DirectX::XMUINT3 clipmap_illumination_texture_extent = brx_voxel_cone_tracing_resource_clipmap_illumination_texture_extent();

		D3D11_TEXTURE3D_DESC texture3d_desc;
		texture3d_desc.Width = clipmap_illumination_texture_extent.x;
		texture3d_desc.Height = clipmap_illumination_texture_extent.y;
		texture3d_desc.Depth = clipmap_illumination_texture_extent.z;
		texture3d_desc.MipLevels = 1U;
		texture3d_desc.Format = DXGI_FORMAT_R32_TYPELESS;
		texture3d_desc.Usage = D3D11_USAGE_DEFAULT;
		texture3d_desc.BindFlags = D3D11_BIND_SHADER_RESOURCE | D3D11_BIND_UNORDERED_ACCESS;
		...
		ID3D11Texture3D *clipmap_texture_illumination = NULL;

		HRESULT res_device_create_texture = device->CreateTexture3D(&texture3d_desc, NULL, &clipmap_texture_illumination);
		assert(SUCCEEDED(res_device_create_texture));
		...
		unordered_access_view_desc.Texture3D.MipSlice = 0U;
		unordered_access_view_desc.Texture3D.FirstWSlice = 0U;
		unordered_access_view_desc.Texture3D.WSize = clipmap_illumination_texture_extent.z;

		assert(NULL == this->m_clipmap_texture_illumination_r32_uav);

		HRESULT res_device_create_unordered_access_view = device->CreateUnorderedAccessView(clipmap_texture_illumination, &unordered_access_view_desc, &this->m_clipmap_texture_illumination_r32_uav);
		assert(SUCCEEDED(res_device_create_unordered_access_view));
		...
		assert(NULL == this->m_clipmap_texture_illumination_r32_srv);

		HRESULT res_device_create_shader_resource_view = device->CreateShaderResourceView(clipmap_texture_illumination, &shader_resource_view_desc, &this->m_clipmap_texture_illumination_r32_srv);
		assert(SUCCEEDED(res_device_create_shader_resource_view));

		clipmap_texture_illumination->Release();
	}

	{
		...
		assert(NULL == this->m_clipmap_texture_illumination_g32_uav);

		HRESULT res_device_create_unordered_access_view = device->CreateUnorderedAccessView(clipmap_texture_illumination, &unordered_access_view_desc, &this->m_clipmap_texture_illumination_g32_uav);
		assert(SUCCEEDED(res_device_create_unordered_access_view));
		...
		assert(NULL == this->m_clipmap_texture_illumination_g32_srv);

		HRESULT res_device_create_shader_resource_view = device->CreateShaderResourceView(clipmap_texture_illumination, &shader_resource_view_desc, &this->m_clipmap_texture_illumination_g32_srv);
		assert(SUCCEEDED(res_device_create_shader_resource_view));
...

	{
		...
		assert(NULL == this->m_clipmap_texture_illumination_b32_uav);

		HRESULT res_device_create_unordered_access_view = device->CreateUnorderedAccessView(clipmap_texture_illumination, &unordered_access_view_desc, &this->m_clipmap_texture_illumination_b32_uav);
		assert(SUCCEEDED(res_device_create_unordered_access_view));
		...
		assert(NULL == this->m_clipmap_texture_illumination_b32_srv);

		HRESULT res_device_create_shader_resource_view = device->CreateShaderResourceView(clipmap_texture_illumination, &shader_resource_view_desc, &this->m_clipmap_texture_illumination_b32_srv);
		assert(SUCCEEDED(res_device_create_shader_resource_view));
...

	{
		...
		assert(NULL == this->m_clipmap_texture_illumination_r16g16b16_uav);

		HRESULT res_device_create_unordered_access_view = device->CreateUnorderedAccessView(clipmap_texture_illumination, &unordered_access_view_desc, &this->m_clipmap_texture_illumination_r16g16b16_uav);
		assert(SUCCEEDED(res_device_create_unordered_access_view));
		...
		assert(NULL == this->m_clipmap_texture_illumination_r16g16b16_srv);

		HRESULT res_device_create_shader_resource_view = device->CreateShaderResourceView(clipmap_texture_illumination, &shader_resource_view_desc, &this->m_clipmap_texture_illumination_r16g16b16_srv);
		assert(SUCCEEDED(res_device_create_shader_resource_view));
...

	{
		...
		rasterizer_desc.DepthBiasClamp = 0.0F;
		rasterizer_desc.SlopeScaledDepthBias = 0.0F;
		rasterizer_desc.DepthClipEnable = FALSE; // The whole primitive will be clipped even if only one of the three Vertices is clipped
		rasterizer_desc.ScissorEnable = FALSE;
	...
	// initialize
	{
		...
		{
			ID3D11UnorderedAccessView *const zero_clipmap_uavs[7] = {this->m_clipmap_texture_mask_uav, this->m_clipmap_texture_opacity_r32_uav, this->m_clipmap_texture_opacity_r16_uav, this->m_clipmap_texture_illumination_r32_uav, this->m_clipmap_texture_illumination_g32_uav, this->m_clipmap_texture_illumination_b32_uav,
																	 this->m_clipmap_texture_illumination_r16g16b16_uav};
		...
		{
			ID3D11UnorderedAccessView *const null_uavs[7] = {NULL, NULL, NULL, NULL, NULL, NULL, NULL};
...
void Demo::Tick(ID3D11DeviceContext *device_context, ID3DUserDefinedAnnotation *user_defined_annotation, IDXGISwapChain *swap_chain)
{
...

	{
		...
		{
			ID3D11UnorderedAccessView *const clear_clipmap_uavs[7] = {this->m_clipmap_texture_mask_uav, this->m_clipmap_texture_opacity_r32_uav, this->m_clipmap_texture_opacity_r16_uav, this->m_clipmap_texture_illumination_r32_uav, this->m_clipmap_texture_illumination_g32_uav, this->m_clipmap_texture_illumination_b32_uav, this->m_clipmap_texture_illumination_r16g16b16_uav};
...

	{
		...
		{
			ID3D11UnorderedAccessView *const voxelization_uavs[5] = {this->m_clipmap_texture_mask_uav, this->m_clipmap_texture_opacity_r32_uav, this->m_clipmap_texture_illumination_r32_uav, this->m_clipmap_texture_illumination_g32_uav, this->m_clipmap_texture_illumination_b32_uav};
		...
		{
			ID3D11UnorderedAccessView *const null_uavs[5] = {NULL, NULL, NULL, NULL, NULL};
...

	{
		...
		{
			ID3D11UnorderedAccessView *const pack_clipmap_uavs[2] = {this->m_clipmap_texture_opacity_r16_uav, this->m_clipmap_texture_illumination_r16g16b16_uav};
			device_context->CSSetUnorderedAccessViews(0U, sizeof(pack_clipmap_uavs) / sizeof(pack_clipmap_uavs[0]), pack_clipmap_uavs, NULL);
		}

		{
			ID3D11ShaderResourceView *const pack_tracing_srvs[5] = {this->m_clipmap_texture_mask_srv, this->m_clipmap_texture_opacity_r32_srv, this->m_clipmap_texture_illumination_r32_srv, this->m_clipmap_texture_illumination_g32_srv, this->m_clipmap_texture_illumination_b32_srv};
		...
		{
			ID3D11UnorderedAccessView *const null_uavs[2] = {NULL, NULL};
			device_context->CSSetUnorderedAccessViews(0U, sizeof(null_uavs) / sizeof(null_uavs[0]), null_uavs, NULL);
		}
...

	{
		...
		{
			ID3D11ShaderResourceView *const cone_tracing_srvs[6] = {this->m_gbuffer_base_color_srv, this->m_gbuffer_roughness_metallic_srv, this->m_gbuffer_normal_srv, this->m_scene_depth_srv, this->m_clipmap_texture_opacity_r16_srv, this->m_clipmap_texture_illumination_r16g16b16_srv};
		...
		{
			ID3D11ShaderResourceView *const null_srvs[6] = {NULL, NULL, NULL, NULL, NULL, NULL};
			device_context->CSSetShaderResources(0U, sizeof(null_srvs) / sizeof(null_srvs[0]), null_srvs);
		}
...
void Demo::UnInit()
{
	...
	this->m_clipmap_texture_opacity_r32_uav->Release();
	this->m_clipmap_texture_opacity_r32_srv->Release();
	this->m_clipmap_texture_opacity_r16_uav->Release();
	this->m_clipmap_texture_opacity_r16_srv->Release();
	this->m_clipmap_texture_illumination_r32_uav->Release();
	this->m_clipmap_texture_illumination_r32_srv->Release();
	this->m_clipmap_texture_illumination_g32_uav->Release();
	this->m_clipmap_texture_illumination_g32_srv->Release();
	this->m_clipmap_texture_illumination_b32_uav->Release();
	this->m_clipmap_texture_illumination_b32_srv->Release();
	this->m_clipmap_texture_illumination_r16g16b16_uav->Release();
	this->m_clipmap_texture_illumination_r16g16b16_srv->Release();
