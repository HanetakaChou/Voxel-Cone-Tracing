...
Demo::Demo()
	: m_shadow_map_dsv(NULL),
	  ...
	  m_clipmap_texture_opacity_uav(NULL),
	  m_clipmap_texture_opacity_srv(NULL),
	  m_clipmap_texture_illumination_uav(NULL),
	  m_clipmap_texture_illumination_srv(NULL),
...
void Demo::Init(ID3D11Device *device, ID3D11DeviceContext *device_context, IDXGISwapChain *swap_chain)
{
...

	{
		...
		device_context->ClearUnorderedAccessViewUint(this->m_clipmap_texture_mask_uav, values);
	}

	{
		...
		HRESULT res_device_create_buffer = device->CreateTexture3D(&texture3d_desc, NULL, &clipmap_texture_opacity);
		assert(SUCCEEDED(res_device_create_buffer));
		...
		assert(NULL == this->m_clipmap_texture_opacity_uav);

		HRESULT res_device_create_unordered_access_view = device->CreateUnorderedAccessView(clipmap_texture_opacity, &unordered_access_view_desc, &this->m_clipmap_texture_opacity_uav);
		assert(SUCCEEDED(res_device_create_unordered_access_view));
		...
		assert(NULL == this->m_clipmap_texture_opacity_srv);

		HRESULT res_device_create_shader_resource_view = device->CreateShaderResourceView(clipmap_texture_opacity, &shader_resource_view_desc, &this->m_clipmap_texture_opacity_srv);
		assert(SUCCEEDED(res_device_create_shader_resource_view));
		...
		// initialize
		UINT values[4] = {0U, 0U, 0U, 0U};
		device_context->ClearUnorderedAccessViewUint(this->m_clipmap_texture_opacity_uav, values);
	}

	{
		DirectX::XMUINT3 clipmap_opacity_texture_extent = brx_voxel_cone_tracing_resource_clipmap_illumination_texture_extent();
		...
		HRESULT res_device_create_buffer = device->CreateTexture3D(&texture3d_desc, NULL, &clipmap_texture_illumination);
		assert(SUCCEEDED(res_device_create_buffer));
		...
		assert(NULL == this->m_clipmap_texture_illumination_uav);

		HRESULT res_device_create_unordered_access_view = device->CreateUnorderedAccessView(clipmap_texture_illumination, &unordered_access_view_desc, &this->m_clipmap_texture_illumination_uav);
		assert(SUCCEEDED(res_device_create_unordered_access_view));
		...
		assert(NULL == this->m_clipmap_texture_illumination_srv);

		HRESULT res_device_create_shader_resource_view = device->CreateShaderResourceView(clipmap_texture_illumination, &shader_resource_view_desc, &this->m_clipmap_texture_illumination_srv);
		assert(SUCCEEDED(res_device_create_shader_resource_view));
...
void Demo::Tick(ID3D11DeviceContext *device_context, ID3DUserDefinedAnnotation *user_defined_annotation, IDXGISwapChain *swap_chain)
{
...
#if 1
	{
		...
		// TODO: the clear is really heavy???

		device_context->CSSetShader(this->m_clear_clipmap_compute_shader, NULL, 0U);

		{
			ID3D11UnorderedAccessView *const clear_clipmap_uavs[3] = {this->m_clipmap_texture_mask_uav, this->m_clipmap_texture_opacity_uav, this->m_clipmap_texture_illumination_uav};
...

	{
		...
		{
			ID3D11UnorderedAccessView *const voxelization_uavs[3] = {this->m_clipmap_texture_mask_uav, this->m_clipmap_texture_opacity_uav, this->m_clipmap_texture_illumination_uav};
...

	{
		...
		{
			ID3D11ShaderResourceView *const cone_tracing_srvs[6] = {this->m_gbuffer_base_color_srv, this->m_gbuffer_roughness_metallic_srv, this->m_gbuffer_normal_srv, this->m_scene_depth_srv, this->m_clipmap_texture_opacity_srv, this->m_clipmap_texture_illumination_srv};
...
void Demo::UnInit()
{
	...
	this->m_clipmap_texture_opacity_uav->Release();
	this->m_clipmap_texture_opacity_srv->Release();
	this->m_clipmap_texture_illumination_uav->Release();
	this->m_clipmap_texture_illumination_srv->Release();
