...
#define BRX_VCT_CONE_TRACING_DIFFUSE_CONE_COUNT 7
#define BRX_VCT_CONE_TRACING_SPECULAR_CONE_COUNT 1
...
brx_bool brx_vct_application_bridge_get_gbuffer_surface(in brx_int2 in_screen_space_coordinates, out brx_float3 out_surface_position_world_space, out brx_float3 out_surface_shading_normal_world_space, out brx_float3 out_surface_diffuse_color, out brx_float3 out_surface_specular_color, out brx_float out_surface_roughness)
{
    ...
    brx_int2 g_screen_width_height = brx_write_only_texture_2d_get_dimension(g_indirect_radiance_and_ambient_occlusion);

    brx_float2 screen_space_uv = (brx_float2(in_screen_space_coordinates) + brx_float2(0.5, 0.5)) / brx_float2(g_screen_width_height);

    brx_float scene_depth = brx_sample_level_2d(g_scene_depth, g_sampler, screen_space_uv, 0).x;
    ...
    else
    {
...

        {
            ...
            brx_float3 base_color = brx_sample_level_2d(g_gbuffer_base_color, g_sampler, screen_space_uv, 0).xyz;

            brx_float2 roughness_metallic = brx_sample_level_2d(g_gbuffer_roughness_metallic, g_sampler, screen_space_uv, 0).xy;

            out_surface_roughness = roughness_metallic.x;

            brx_float metallic = roughness_metallic.y;

            out_surface_shading_normal_world_space = brx_octahedral_unmap(brx_sample_level_2d(g_gbuffer_normal, g_sampler, screen_space_uv, 0).xy);
...
{
#if 1
    brx_int3 opacity_texture_dimension = brx_texture_3d_get_dimension(g_clipmap_texture_opacity, 0);

    brx_float3 normalized_opacity_texture_coordinates = in_opacity_texture_coordinates / brx_float3(opacity_texture_dimension);

    brx_float opacity = brx_sample_level_3d(g_clipmap_texture_opacity, g_sampler, normalized_opacity_texture_coordinates, 0).x;
#else
    brx_float opacity = brx_uint_as_float(brx_fetch_3d(g_clipmap_texture_opacity, brx_int4(brx_round(in_opacity_texture_coordinates), 0))).x;
#endif

    return opacity;
}

brx_float brx_vct_application_bridge_get_clipmap_illumination(in brx_float3 in_illumination_texture_coordinates)
{
#if 1
    brx_int3 illumination_texture_dimension = brx_texture_3d_get_dimension(g_clipmap_texture_illumination, 0);

    brx_float3 normalized_illumination_texture_coordinates = in_illumination_texture_coordinates / brx_float3(illumination_texture_dimension);

    brx_float illumination_channel = brx_sample_level_3d(g_clipmap_texture_illumination, g_sampler, normalized_illumination_texture_coordinates, 0).x;
#else
    brx_float illumination_channel = brx_uint_as_float(brx_fetch_3d(g_clipmap_texture_illumination, brx_int4(brx_round(in_illumination_texture_coordinates), 0))).x;
#endif
    return illumination_channel;
}
...
{
    brx_store_2d(g_indirect_radiance_and_ambient_occlusion, in_screen_space_coordinates, in_radiance_and_ambient_occlusion);
}
