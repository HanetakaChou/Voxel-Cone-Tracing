...
Texture2D g_gbuffer_base_color : register(t0);
Texture2D g_gbuffer_roughness_metallic : register(t1);
Texture2D g_gbuffer_normal : register(t2);
Texture2D g_scene_depth : register(t3);

Texture3D g_clipmap_texture_opacity : register(t4);
Texture3D g_clipmap_texture_illumination : register(t5);

SamplerState g_sampler : register(s0);

RWTexture2D<float4> g_indirect_radiance_and_ambient_occlusion : register(u0);
